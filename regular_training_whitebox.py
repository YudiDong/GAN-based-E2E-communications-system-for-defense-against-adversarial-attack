...
BLER_No_Attack, BLER_Adv_Attack_minus6, BLER_Jamming_Attack_minus6 = ae_MLP.bler_sim_attack_AWGN(universal_per_fgm_MLP.reshape(1,2,n), PSR_dB, ebnodbs, batch_size, 300) #100
...
ax.plot(ebnodbs,BLER_Adv_Attack_minus6,'r^-',label='Adversarial Attack (Autoencoder)')

ax.plot(ebnodbs,BLER_HD_adv,'g*--',label='Adversarial Attack (BPSK, Hamming)')
ax.plot(ebnodbs,BLER_HD,'s--',label='No Attack (BPSK, Hamming)')
ax.plot(ebnodbs,BLER_No_Attack,'ko-',label='No Attack (Autoencoder)')
...
with open('white_attack_AE-BPSK', 'wb') as f:  # Python 3: open(..., 'wb')
